...
function allfapai(data){
        ...
        for(var i=0;i<data.user.length;i++){
            ...
            html=html+'<div class="cardss cards card'+indexuser+'" style="display: none;" data="'+indexuser+'"><div class="card card'+indexuser+'1 sangong6-index1" ></div> <div class="card card'+indexuser+'2 sangong6-index2" ></div> <div class="card card'+indexuser+'3 sangong6-index3" ></div></div>';
            $('#userfp').append(html);
        }
        $('#userfp .cards').show();
        setTimeout(function(){
...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:193"+sj.id, 185,293);
        ctx.fillText(time1, 400, 293);
