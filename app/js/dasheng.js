...
var Page = {
    ...
    moreFunc: function() {
        var code = '<div class="more-mask2"><div class="content-mask"><div class="content-box"><i class="mask-icon mask-top"></i><i class="mask-icon mask-right"></i><i class="mask-icon mask-bottom"></i><i class="mask-icon mask-left"></i><div class="content-item"><div class="item switch-platform">切换平台</div></div><div class="content-item"><div class="item user-friend">好友管理</div></div><div class="content-item"><div class="item pk-history">历史战绩</div></div><!--<div class="content-item"><div class="item user-update">更新资料</div></div>--><div class="content-item"><div class="item more-room-card">房卡明细</div></div><div class="content-item"><div class="item find-account">找回账户</div></div><div class="content-item"><div class="item log-out">注销登录</div></div><div class="content-item last"><div class="item error">异常举报</div></div><div class="close-more"></div></div></div></div>';
        $(code).appendTo('.index');
    },
    picUpload: function() {
    ...
    load: function() {
        ...
        Page.goldFlowerSwitch('.goldflower-mask');
    ...
    pushBobbinGame: function() {
        $('.game-list .pushBobbin-bg').one('click',
            function() {
                ...
                $('.pushBobbin-mask .tab span[data-item="1"]').touch();
    ...
    pushBobbinSwitch: function(className) {
        $(className + ' .tab span').touch(function() {
            ...
            var config = selectConfig['28'][item-1];
            ...
            if(config["df"]) {
                ...
                $(className + ' .content > .flex-cont.number > .flex-item')
                    .find(".prev,.next,.prev2,.next2,.flex-item > div,.prev,.next,.prev2,.next2,.flex-item > span")
                    .click(function(e) {
            ...
            $(className + ' .content > .chouma').empty().css("display", "none");
            if(config['cm']) {
                var cm_html = '<div class="name">' +
                    '筹码:' +
                    '</div>' +
                    '<div class="flex-item">';
                var cm = config['cm'].split(",");
                for (var i = 0; i < cm.length; i ++) {
                    if(i!=0) cm_html += '<br />';
                    cm_html += '<label onclick="send(\'rule\',{id:\'gz\',key:'+i+'})"><input type="radio" name="push_bobbin_chouma" value="1" '+(i == 0 ? 'checked' : '')+' /><span>'+cm[i]+'</span></label>';
                }
                cm_html += '</div>';
                $(className + ' .content > .chouma').css("display", "flex").html(cm_html);
            }

            $(className + ' .content > .rules').empty();
            if(config['sx']) {
                var sx_html = '<div class="name">' +
                    '                                下注上限:' +
                    '                            </div>' +
                    '                            <div class="flex-item">';
                var sx = config['sx'].split(",");
                for (var i = 0; i < sx.length; i ++) {
                    if(i!=0) sx_html += '<br />';
                    sx_html += '<label onclick="send(\'rule\',{id:\'gz\',key:'+i+'})"><input type="radio" name="push_bobbin_rule" value="1" '+(i == 0 ? 'checked' : '')+' /><span>'+sx[i]+'</span></label>'+" \n";
                }
                sx_html += '</div>';
                $(className + ' .content > .rules').html(sx_html);
            }
            ...
            $(className + ' .content > .shang_zhuang').css("display", "none").empty();
            if(config['sz']) {
                var sz = config['sz'].split(",");
                var zhuang = '<div class="name">上庄:</div>';
                zhuang += '<div class="flex-item">';
                for(var i = 0; i < sz.length; i ++) {
                    zhuang += '<label onclick="send(\'rule\',{id:\'sz\',key:'+i+'})"><input type="radio" name="push_bobbin_shang_zhuang" value="0" '+(i==0 ? 'checked' : '')+'><span>'+sz[i]+'</span></label>';
                }
                zhuang += '</div>';
                $(className + ' .content > .shang_zhuang').css("display", "flex").html(zhuang);
            }
        });
    },
    douniuGame: function() {
    ...
    goldFlowerSwitch: function(className) {
        var config = selectConfig['flower'][0];
        $(className + ' .radio >div').removeClass('on');
        if(config["df"]) {
            var df = config["df"].split(",");
            if(df.length < 4) {
        ...
        $(className + ' .content > .rules').empty();
        if(config['cm']) {
            var zd_html = '<div class="name">' +
                '                                分数:' +
                '                            </div>' +
                '                            <div class="flex-item">';
            var zd = config['cm'].split(",");
            for (var i = 0; i < zd.length; i ++) {
                if(i!=0) zd_html += '<br />';
                zd_html += '<label onclick="send(\'rule\',{id:\'cm\',key:'+i+'})"><input type="radio" name="niuniuTen_card_rule" value="1" '+(i == 0 ? 'checked' : '')+' /><span>'+zd[i]+'</span></label>'+" \n";
            }
            zd_html += '</div>';
            $(className + ' .content > .rules').html(zd_html);
        }
        ...
        $(className + ' .content > .goldflower-number').empty();
        if(config['sx']) {
            var sx = config['sx'].split(",");
            var zhuang = '<div class="flex-cont ">';
            zhuang += '    <div class="name">上限:</div>';
            zhuang += '    <div class="flex-item">';
            for(var i = 0; i < sx.length; i ++) {
                zhuang += '<label onclick="send(\'rule\',{id:\'sx\',key:'+i+'})"><input type="radio" name="niuniuTen_zhuang_value" value="0" '+(i==0 ? 'checked' : '')+'><span>'+sx[i]+'</span></label>';
            }
            zhuang += '   </div>';
            zhuang += '</div>';
            $(className + ' .content > .goldflower-number').html(zhuang);
        }
    },
    ...
    douniuThirteenGame: function() {
        $('.game-list .thirteenoxen-bg ').one('click',
            function() {
                ...
                $('.niuniuThirteen-mask .tab span[data-item="1"]').touch();
                if (data) {
    ...
    douniuThirteenSwitch: function(className) {
        $(className + ' .tab span').touch(function() {
            var item = parseInt($(this).data('item'));
            var config = selectConfig['13niu'][item-1];
            $(this).siblings(className + ' .tab-item').removeClass('on');
            $(this).addClass('on');
            console.log(config);
