...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText(window.fangzhu.skinname+"房间:"+sj.id, 175,192);
        ctx.fillText(time1, 375, 192);
        ctx.fillText(sj.zjs+"局", 575, 192);
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.drawImage(img2,105,286-(0-(i-1)*70),613,65);
            }
            var user=sj['user'][i];
            ctx.font = "31px bolder songti";
            ctx.fillStyle = "black";
            ctx.fillText(user.id, 175, 270+(i*67));
            ctx.fillText(decode64(user.nickname).substring(0,9), 319, 270+(i*67));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }
            ctx.fillText( user.dqjf, 581, 270+(i*67));
        }
