...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText(window.fangzhu.skinname+"房间:"+sj.id, 175,195);
        ctx.fillText(time1, 375, 195);
        ctx.fillText(sj.zjs+"局", 575, 195);
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.drawImage(img2,105,299-(0-(i-1)*79),590,75);
            }
            var user=sj['user'][i];
            ctx.font = "31px bolder songti";
            ctx.fillStyle = "black";
            ctx.fillText(user.id, 175, 270+(i*79));
            ctx.fillText(decode64(user.nickname).substring(0,9), 319, 270+(i*79));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }
            ctx.fillText( user.dqjf, 581, 270+(i*79));
        }
